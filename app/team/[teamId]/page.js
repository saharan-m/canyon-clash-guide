...
export default async function TeamPage({ params }) {
  ...
  return (
    <main className="min-h-screen bg-animated-gradient relative overflow-hidden crt">
      ...
      <div className="relative z-10 p-4 sm:p-6 md:p-8">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-2 mb-6 sm:mb-8 text-muted hover:text-white transition-all duration-300 ease-out font-semibold text-sm sm:text-base group cursor-pointer focus:outline-none focus:ring-2 focus:ring-purple-400"
        ...
        <div className="card-glow p-5 sm:p-6 md:p-8 mb-6 sm:mb-8 relative overflow-hidden border-2" style={{ borderColor: team.color, boxShadow: `0 0 24px ${team.color}80, 0 0 48px ${team.color}40` }}>
          <div 
          ...
          <div className="flex flex-row items-center gap-3 sm:gap-4 md:gap-6">
            ...
            <div
              ...
              aria-label={`Team ${team.name} icon`}
            >
              <TeamIcon team={team} size={20} className="text-white w-5 h-5 sm:w-7 sm:h-7 md:w-8 md:h-8 lg:w-10 lg:h-10" />
            </div>
            <div className="flex-1 min-w-0">
              <h1 className="heading-2 mb-2 sm:mb-3" style={{ color: '#E2E8F0', textShadow: '0 0 8px #7C3AED, 0 0 16px #F43F5E, 0 1px 2px #0F0F23', fontFamily: 'Russo One, Chakra Petch, sans-serif', fontWeight: 900, letterSpacing: '0.01em' }}>{team.name}</h1>
              {/* Team Members - Prominently Displayed */}
              {team.members.length > 0 && (
                <div className="mb-3 sm:mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    <svg className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: team.color }} fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                  ...
                  <div className="flex flex-wrap gap-2 sm:gap-3">
                    {team.members.map((member, idx) => (
                      <div 
                        ...
                        style={{ backgroundColor: `${team.color}20`, borderColor: `${team.color}40`, color: '#ffffff' }}
                        aria-label={`Team member: ${member}`}
                      >
                        {member}
                      </div>
                    ))}
              ...
              <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                {team.path.length > 0 && (
                  <div className="flex items-center gap-2">
                    <svg className="w-4 h-4 sm:w-5 sm:h-5 text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            ...
            {team.path.length > 0 && (
              <div className="card-glow p-4 sm:p-5 md:p-6">
                <div className="flex items-center gap-3 mb-4 sm:mb-6 pb-3 border-b border-slate-700/50">
                  ...
                  <h2 className="heading-3" style={{ color: '#E2E8F0', textShadow: '0 0 8px #7C3AED, 0 0 16px #F43F5E, 0 1px 2px #0F0F23', fontFamily: 'Russo One, Chakra Petch, sans-serif', fontWeight: 700, letterSpacing: '0.01em' }} data-text="Your Path on the Map">Your Path on the Map</h2>
                </div>
